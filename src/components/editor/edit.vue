          ...
          toolbar: {
            ...
            handlers: {
              image: function(value) {
                if (value) {
                  // 触发input框选择图片文件
                  document.querySelector(".editorBox .avatar-uploader input").click();
                } else {
  ...
  methods: {
    ...
    uploadSuccess(file) {// res为图片服务器返回的数据
	  ...
	  uploadImg(formData).then(res=>{
	  		  if (res.code == 200) {
				let imgUrl=process.env.NODE_ENV == 'development' ? this.Global.devUrl : this.Global.proUrl
	  		    let url = imgUrl+res.msg
